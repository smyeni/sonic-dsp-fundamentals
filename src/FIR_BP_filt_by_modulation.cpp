...
// ------------------------------------------------------------------
// Time domain modulator
// ------------------------------------------------------------------
[[nodiscard]]
std::vector<double> modulator(std::size_t N, const double fc, const double Fs)
{
    std::vector<double> m(N);
    for (std::size_t n = 0; n < N; ++n)
        m[n] = 2 * std::cos(2.0 * std::numbers::pi * fc * n / Fs );
    return m;
}
...
int main(int argc, char* argv[])
{
    ...
    const double fc = 1500;

    // --------------------------------------------------------------
    // 1. Ideal frequency response
    // --------------------------------------------------------------
    CVec H(N, 0.0);
    ...
    //---------------------------------------------------------------
    // 6 Modulate by fc = 1500Hz
    //---------------------------------------------------------------
    std::vector<double> mod_vector = modulator( N, fc, Fs );
    CVec h_bpass_win;
    for (std::size_t n = 0; n < N; ++n)
    {
        h_bpass_win.push_back( mod_vector[n] * h_win[n] );
    }

    // --------------------------------------------------------------
    // 7. Write CSV files (same layout you already use)
    // --------------------------------------------------------------
    {
    ...
    {
        std::ofstream f("h_bpass_windowed.csv");
        std::println(f, "k,h_real,h_imag,hamming");
        for (std::size_t k = 0; k < N; ++k)
        {
            std::println(f, "{},{},{},{}",
                         k, h_bpass_win[k].real(), h_bpass_win[k].imag(), w[k]);
        }
    }

    // --------------------------------------------------------------
    // 8. Launch the plotter (same as before)
    // --------------------------------------------------------------
